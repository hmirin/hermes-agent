...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # ── Version 31 → 32: add opt-in Codex-native compaction gate ──
    # Keep existing installs on Hermes' summary compressor by default. Users can
    # opt in to Codex Responses/app-server compaction explicitly.
    if current_ver < 32:
        config = read_raw_config()
        comp = config.get("compression")
        if not isinstance(comp, dict):
            comp = {}
        if "codex_native_compaction" not in comp:
            comp["codex_native_compaction"] = False
            config["compression"] = comp
            save_config(config)
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    "  ✓ Added compression.codex_native_compaction: false "
                    "(set true to opt in to Codex-native compaction)"
                )
